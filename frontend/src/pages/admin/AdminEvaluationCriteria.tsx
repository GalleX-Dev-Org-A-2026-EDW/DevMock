...
import { useEvaluationCriteria, useCreateEvaluationCriterion, useUpdateEvaluationCriterion, useDeleteEvaluationCriterion } from "@/api/evaluation-criteria.queries"
import type { EvaluationCriterion, CreateEvaluationCriterionDto, UpdateEvaluationCriterionDto } from "@/api/evaluation-criteria"
...
export default function AdminEvaluationCriteria() {
  ...
  const { data: items, isLoading, isError } = useEvaluationCriteria()
  const createItem = useCreateEvaluationCriterion()
  const updateItem = useUpdateEvaluationCriterion()
  const deleteItem = useDeleteEvaluationCriterion()
  ...
  const validate = (): boolean => {
    ...
    if (!form.defaultWeight || form.defaultWeight <= 0 || form.defaultWeight > 100) next.defaultWeight = "El peso debe estar entre 1 y 100"
    setErrors(next)
  ...
  const openCreate = () => { setErrors({}); setEditing(null); setForm({ name: "", slug: "", defaultWeight: 25 }); setShowModal(true) }
  const openEdit = (item: EvaluationCriterion) => { setErrors({}); setEditing(item); setForm({ name: item.name, slug: item.slug, description: item.description ?? undefined, defaultWeight: item.defaultWeight }); setShowModal(true) }

  const handleSave = async () => {
    if (!validate()) return
    if (editing) { await updateItem.mutateAsync({ id: editing.id, dto: form as UpdateEvaluationCriterionDto }) }
  ...
  return (
    <div>
      <div className="flex items-center flex-wrap gap-3 mb-6">
        <div className="h-7 w-1 rounded-full bg-gradient-to-b from-rose-400 to-rose-600" />
        <h1 className="font-['Work_Sans'] font-bold text-xl sm:text-2xl text-white">Criterios de Evaluación</h1>
        <div className="ml-auto">
          <button onClick={openCreate} className="px-4 py-2 bg-emerald-500 text-white rounded-lg text-sm font-semibold hover:bg-emerald-600 transition-all flex items-center gap-2"><Plus className="h-4 w-4" /> Nuevo criterio</button>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.06] bg-white/[0.03]">
                ...
                <th className="text-center px-4 py-3 font-semibold text-white/50">Peso default</th>
            ...
            <tbody>
              {paginatedItems?.map((item) => (
                <tr key={item.id} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors">
                  ...
                  <td className="px-4 py-3 text-center font-['JetBrains_Mono'] text-white/90">{item.defaultWeight}%</td>
              ...
              {(!items || items.length === 0) && <tr><td colSpan={6} className="px-4 py-8 text-center text-white/40">No hay criterios de evaluación</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {items && items.length > PAGE_SIZE && (
        <div className="flex items-center justify-between mt-4 px-1">
          <p className="text-sm text-white/40">
            {items.length} criterios — Página {currentPage} de {totalPages}
          </p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={closeModal}>
          <div className="bg-[#1e293b] rounded-2xl p-6 sm:p-8 w-full max-w-md shadow-2xl border border-white/[0.08]" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-['Work_Sans'] font-bold text-xl text-white mb-6">{editing ? "Editar criterio" : "Nuevo criterio"}</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-white/80 mb-1">Peso por defecto (%) <span className="text-red-400">*</span></label>
                <input type="number" min="0" max="100" value={form.defaultWeight} onChange={(e) => { setForm({ ...form, defaultWeight: parseInt(e.target.value) || 0 }); clearError("defaultWeight") }} className={`w-full px-3 py-2.5 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary bg-white/[0.05] text-white placeholder:text-white/30 ${errors.defaultWeight ? "border-red-400" : "border-white/[0.1]"}`} />
                {errors.defaultWeight && <p className="text-red-400 text-xs mt-1">{errors.defaultWeight}</p>}
              </div>
              <div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setConfirmDelete(null)}>
          <div className="bg-[#1e293b] rounded-2xl p-6 sm:p-8 w-full max-w-sm shadow-2xl border border-white/[0.08]" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-['Work_Sans'] font-bold text-xl text-white mb-2">¿Eliminar criterio?</h2>
