  ...
  return (
    <div>
      <div className="flex items-center flex-wrap gap-3 mb-6">
        ...
        <h1 className="font-['Work_Sans'] font-bold text-xl sm:text-2xl text-white">Logros</h1>
            ...
            <tbody>
              {paginatedItems?.map((item) => (
                <tr key={item.id} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors">
                  ...
                  <td className="px-4 py-3 text-center font-medium text-white/90">{item.pointsReward ?? "-"}</td>
              ...
              {(!items || items.length === 0) && <tr><td colSpan={6} className="px-4 py-8 text-center text-white/40">No hay logros</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {items && items.length > PAGE_SIZE && (
        <div className="flex items-center justify-between mt-4 px-1">
          <p className="text-sm text-white/40">
            {items.length} logros — Página {currentPage} de {totalPages}
          </p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={closeModal}>
          <div className="bg-[#1e293b] rounded-2xl p-6 sm:p-8 w-full max-w-lg shadow-2xl border border-white/[0.08]" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-['Work_Sans'] font-bold text-xl text-white mb-6">{editing ? "Editar logro" : "Nuevo logro"}</h2>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-white/80 mb-1">Puntos de recompensa <span className="text-red-400">*</span></label>
                  <input type="number" value={form.pointsReward ?? ""} onChange={(e) => { setForm({ ...form, pointsReward: e.target.value ? parseInt(e.target.value) : undefined }); clearError("pointsReward") }} className={`w-full px-3 py-2.5 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary bg-white/[0.05] text-white placeholder:text-white/30 ${errors.pointsReward ? "border-red-400" : "border-white/[0.1]"}`} />
                  {errors.pointsReward && <p className="text-red-400 text-xs mt-1">{errors.pointsReward}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-white/80 mb-1">URL del icono</label>
                  <input type="text" value={form.iconUrl || ""} onChange={(e) => setForm({ ...form, iconUrl: e.target.value })} className="w-full px-3 py-2.5 rounded-lg border border-white/[0.1] text-sm focus:outline-none focus:ring-2 focus:ring-primary bg-white/[0.05] text-white placeholder:text-white/30" />
                </div>
              </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setConfirmDelete(null)}>
          <div className="bg-[#1e293b] rounded-2xl p-6 sm:p-8 w-full max-w-sm shadow-2xl border border-white/[0.08]" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-['Work_Sans'] font-bold text-xl text-white mb-2">¿Eliminar logro?</h2>
